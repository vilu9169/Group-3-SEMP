...
class Board:
    ...
    def populate_from_state(self, coord, player, does_stand, player_1_color):
        square = self.get_square_from_coord(coord)
        piece = self.get_piece_from_pos(coord)
        ...
        if square.is_valid_coordinate(coord):
            ...
            if player == 1:
                if player_1_color == "blue":
                    ...
                    self.piecesleft_red -= 1
                square.occupying_piece  = Piece(coord, player_1_color, self, does_stand)
                square.pieces.append(Piece(coord, player_1_color, self, does_stand))

            else:
                if player_1_color == "blue":
                    self.piecesleft_red -= 1
                    square.occupying_piece  = Piece(coord, "red", self, does_stand)
                    square.pieces.append(Piece(coord, "red", self, does_stand))
                else:
                    self.piecesleft_blue -= 1
                    square.occupying_piece  = Piece(coord, "blue", self, does_stand)
                    square.pieces.append(Piece(coord, "blue", self, does_stand))

            return True
        return False;


    def fill_square_from_state(self, coord, piece_list, player_1_color):
        print(piece_list)
        piece_id = 1
        is_standing = 0
        num_pieces = len(piece_list)- 1
        for piece in piece_list[1:]:
            if piece_list[0] == 1 and piece_id == num_pieces:
                is_standing = 1
            player = piece - 1
            self.populate_from_state(coord, player, is_standing, player_1_color)
            piece_id += 1
        return True

    def fill_board_from_state(self, board, player_1_color):
        for row in range(4):
            for col in range(4):
                self.fill_square_from_state((row, col), board[col][row], player_1_color) # Notice that we do board[col][row] since our implementations are flipped
        return True


    def extract_state_from_board(self, player_1_color):
        state = []
        for row in range(4):
            curr_row = []
            for col in range(4):
                coord = (col, row)
                square = self.get_square_from_coord(coord)
                square_pieces = square.get_pieces(player_1_color)
                curr_row.append(square_pieces)
