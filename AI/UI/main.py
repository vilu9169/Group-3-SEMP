...
example_board = [[[1, 2], [0, 3], [1, 3, 2], [0]],
                 [[0, 2, 2], [1, 3, 3], [0, 3, 2], [0, 2, 2]],
                 [[1, 3], [0, 2], [0, 2, 2, 2], [0, 3, 3]],
                 [[1, 3, 2], [1, 2, 3], [0, 3], [1, 3, 2]]]

example_board2 = [[[0, 2],   [0],        [0],           [0]],
                 [[0, 2, 2], [1, 3, 3],  [0, 3, 2],     [0, 2, 2]],
                 [[1, 3],    [0, 2],     [0, 2, 2, 2],  [0, 3, 3]],
                 [[1, 3, 2], [1, 2, 3],  [0, 3],        [1, 3, 2]]]
...
def main():
    ...
    #board.fill_board_from_state(example_board)
    ...
    while run:
        ...
        if game_state == GameInit.BLUE:
            #board.fill_board_from_state(example_board2, "blue")
            run = generate_board(screen, board, "blue",i)
            game_state = GameInit.BLUE


        if game_state == GameInit.RED:
            #board.fill_board_from_state(example_board2, "red")
            run = generate_board(screen, board, "red",i)
            state = board.extract_state_from_board("red")
            print_board(state)
